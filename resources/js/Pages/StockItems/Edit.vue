...
const form = ref({
    ...
    unit: props.stockItem.unit ?? '',
    barcode: props.stockItem.barcode ?? '',
    expiry_date: props.stockItem.expiry_date ?? '',
    current_stock: props.stockItem.current_stock ?? '',
    reorder_level: props.stockItem.reorder_level ?? '',
    cost_price: props.stockItem.cost_price ?? '',
    sale_price: props.stockItem.sale_price ?? '',
    is_active: props.stockItem.is_active ?? true,
});

const errors = ref({});

function submit() {
    router.put(`/portal/stock-items/${props.stockItem.id}`, form.value, {
...
<template>
    <AppLayout>
        <Panel eyebrow="Stock" title="Edit stock item">
