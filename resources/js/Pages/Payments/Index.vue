        ...
        <div class="space-y-7">
            ...
            >
                <div>
                    ...
                    <p class="text-gms-text-muted mt-1 font-medium">Financial overview and transaction history.</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <div
                    ...
                    class="group relative overflow-hidden rounded-3xl bg-white border border-gms-border p-6 shadow-sm hover:border-[#FF6B35] transition-all hover:-translate-y-0.5"
                >
                    ...
                    <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-gms-text-muted">This Month</p>
                    <div class="mt-2 text-3xl font-black text-gms-text">{{ formatCurrency(summary.thisMonthRevenue) }}</div>
                    <p class="mt-2 text-[11px] font-medium text-gms-text-muted">Revenue in {{ new Date().toLocaleString('default', { month: 'long' }) }}</p>
                </div>
                <div
                    v-motion
                    :initial="{ opacity: 0, y: 20 }"
                ...
                >
                    ...
                    <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-gms-text-muted">Transactions</p>
                    <div class="mt-2 text-3xl font-black text-gms-text">{{ summary.totalTransactions ?? 0 }}</div>
                    <p class="mt-2 text-[11px] font-medium text-gms-text-muted">All time</p>
                </div>
                <div
                    v-motion
                    :initial="{ opacity: 0, y: 20 }"
                ...
                >
                    ...
                    <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-gms-text-muted">Active Plans</p>
                    <div class="mt-2 text-3xl font-black text-gms-text">{{ summary.activePlans ?? 0 }}</div>
                    <p class="mt-2 text-[11px] font-medium text-gms-text-muted">Currently subscribed</p>
                </div>
            </div>
            ...
            <div class="grid gap-6 xl:grid-cols-12">
                ...
                <div
                    ...
                    class="rounded-3xl border border-gms-border bg-white shadow-sm xl:col-span-8"
                >
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-[#F5F5F5]">
                                ...
                                >
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-medium text-gms-text">{{ payment.member?.first_name }} {{ payment.member?.last_name }}</p>
                                                <p class="text-[10px] font-bold uppercase tracking-wider text-gms-text-muted">{{ payment.member?.member_code }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-gms-text-muted font-medium">{{ formatDate(payment.paid_at) }}</td>
                                    <td class="px-6 py-4">
                                ...
                                <tr v-if="!recentPayments.length">
                                    <td colspan="5" class="px-6 py-10 text-center text-gms-text-muted font-medium">No transactions yet.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div
                    ...
                    class="rounded-3xl border border-gms-border bg-white shadow-sm xl:col-span-4 p-6"
                >
                    ...
                    <div class="space-y-4">
                        <div v-for="plan in revenueByPlan" :key="plan.name" class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gms-text">{{ plan.name }}</span>
                            ...
                            <p class="text-[11px] font-medium text-gms-text-muted">{{ plan.count }} subscriptions</p>
                        </div>
                        <div v-if="!revenueByPlan.length" class="py-6 text-center text-gms-text-muted font-medium text-sm">
                            No revenue data yet.
                        </div>
                    </div>
                </div>
