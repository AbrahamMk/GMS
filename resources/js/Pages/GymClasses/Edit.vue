...
const props = defineProps({
    gymClass: { type: Object, required: true },
});

const form = useForm({
    name: props.gymClass.name ?? '',
    description: props.gymClass.description ?? '',
    trainer_user_id: props.gymClass.trainer_user_id ?? '',
    capacity: props.gymClass.capacity ?? '',
    duration_minutes: props.gymClass.duration_minutes ?? '',
    is_active: props.gymClass.is_active ?? true,
});

function submit() {
    form.put(`/portal/gym-classes/${props.gymClass.id}`);
}
</script>
...
<template>
    <AppLayout>
        <div class="mx-auto max-w-2xl">
            <Panel eyebrow="Gym Classes" title="Edit class">
