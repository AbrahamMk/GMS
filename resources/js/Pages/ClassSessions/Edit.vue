...
const form = useForm({
    gym_class_id: props.classSession.gym_class_id ?? '',
    starts_at: props.classSession.starts_at ?? '',
    ends_at: props.classSession.ends_at ?? '',
    capacity_override: props.classSession.capacity_override ?? '',
    status: props.classSession.status ?? 'scheduled',
});

function submit() {
    form.put(`/portal/class-sessions/${props.classSession.id}`);
}
</script>
...
<template>
    <AppLayout>
        <div class="mx-auto max-w-2xl">
            <Panel eyebrow="Class Sessions" title="Edit session">
