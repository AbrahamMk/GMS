...
const form = ref({ name: props.category.name ?? '' });
const errors = ref({});

function submit() {
    router.put(`/portal/equipment-categories/${props.category.id}`, form.value, {
...
<template>
    <AppLayout>
        <Panel eyebrow="Assets" title="Edit equipment category">
